...
describe('uploadArtifacts', () => {
  ...
  describe('#uploadZipFile()', () => {
    ...
    it('should upload the .zip with the highest version number while using patterns', () => {
      const tmpDirPath = testUtils.getTmpDirPath();
      const artifactFilePathPattern = path.join(tmpDirPath, 'artifact-*.zip');
      const artifactFilePathOne = path.join(tmpDirPath, 'artifact-1.zip');
      const artifactFilePathTwo = path.join(tmpDirPath, 'artifact-2.zip');

      serverless.utils.writeFileSync(artifactFilePathOne, 'artifact-1.zip file content');
      serverless.utils.writeFileSync(artifactFilePathTwo, 'artifact-2.zip file content');

      return awsDeploy.uploadZipFile(artifactFilePathPattern).then(() => {
        expect(readFileSyncStub).to.have.been.calledWithExactly(artifactFilePathTwo);
        expect(readFileSyncStub).to.not.have.been.calledWithExactly(artifactFilePathOne);
      });
    });

    it('should upload .zip without version number if second one with number is existent', () => {
      const tmpDirPath = testUtils.getTmpDirPath();
      const artifactFilePathPattern = path.join(tmpDirPath, 'artifact*.zip');
      const artifactFilePathOne = path.join(tmpDirPath, 'artifact.zip');
      const artifactFilePathTwo = path.join(tmpDirPath, 'artifact-2.zip');

      serverless.utils.writeFileSync(artifactFilePathOne, 'artifact.zip file content');
      serverless.utils.writeFileSync(artifactFilePathTwo, 'artifact-2.zip file content');

      return awsDeploy.uploadZipFile(artifactFilePathPattern).then(() => {
        expect(readFileSyncStub).to.have.been.calledWithExactly(artifactFilePathOne);
        expect(readFileSyncStub).to.not.have.been.calledWithExactly(artifactFilePathTwo);
      });
    });
