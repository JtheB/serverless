...
module.exports = {
  ...
  uploadZipFile(artifactFilePath) {
    let tmpArtifactFilePath = artifactFilePath;

    if (globby.hasMagic(tmpArtifactFilePath)) {
      tmpArtifactFilePath = globby.sync(tmpArtifactFilePath);
      ...
      if (tmpArtifactFilePath.length !== 1) {
        tmpArtifactFilePath = tmpArtifactFilePath[tmpArtifactFilePath.length - 1].toString();
      } else {
        tmpArtifactFilePath = artifactFilePath[0].toString();
      }
    }

    const fileName = tmpArtifactFilePath.split(path.sep).pop();

    // TODO refactor to be async (use util function to compute checksum async)
    const data = fs.readFileSync(tmpArtifactFilePath);
    ...
    let params = {
      ...
      Body: fs.createReadStream(tmpArtifactFilePath),
